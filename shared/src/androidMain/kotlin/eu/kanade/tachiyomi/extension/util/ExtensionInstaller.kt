 ...
 *
 * Vendored from Mihon, adapted: Folio always uses Mihon's PRIVATE install mode, where the
 * downloaded extension APK is stored in the app's private data directory and loaded via a
 * delegate-last classloader. This needs no system installer permission and works on every
...
class ExtensionInstaller(
    ...
    fun downloadAndInstall(
        url: String,
        extension: Extension,
        @Suppress("UNUSED_PARAMETER") isUpdateForPrivatelyInstalled: Boolean = false,
    ): Flow<InstallStep> {
        ...
        val job = scope.launch {
            ...
            try {
                ...
                installApkPrivately(downloadId, tmpFile)
            } catch (e: Exception) {
