...
/**
 * The built-in reader themes, in picker order. Pure data, split out of
 * ReaderSettings.kt so that file stays within its §6 budget.
 */
internal val ReaderThemePresets: Map<String, Theme> = mapOf(
    // ── LIGHT READER THEMES ─────────────────────────────────────
    "white" to Theme(
        id = "white", name = "White",
    ...
    "dawn" to Theme(
        id = "dawn", name = "Dawn",
        background = 0xFFF7F2FC.toInt(), surface = 0xFFECE2F7.toInt(),
        primaryText = 0xFF2B1B41.toInt(), secondaryText = 0xFF6A5A85.toInt(),
        headingText = 0xFF6D28D9.toInt(), link = 0xFF7C3AED.toInt(),
        selection = 0xFFD8C6F2.toInt(), bookmark = 0xFFD6337A.toInt(),
        highlightColors = listOf(
            0xFFE8C84D.toInt(), 0xFF4D8FC7.toInt(), 0xFF5DAE5D.toInt(), 0xFFD65F5F.toInt(),
            0xFF9B7BC7.toInt(), 0xFFD4A843.toInt(), 0xFF5DB8B8.toInt(), 0xFFD67AB5.toInt()
        ),
        progress = 0xFF7C3AED.toInt(), divider = 0xFFE0D5EE.toInt(),
        isDark = false
    ),
    // ── DARK READER THEMES ──────────────────────────────────────
    "dark" to Theme(
